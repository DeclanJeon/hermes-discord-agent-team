...
채널을 새로 생성할 때 각 채널에는 다음이 자동으로 설정됩니다.
- 채널 topic에 역할별 이모지와 한 줄 설명
- 첫 메시지로 채널 목적, 담당 역할, 작성해야 할 내용, 운영 원칙 안내
- 기존 채널은 중복 안내 메시지 스팸을 피하기 위해 건너뜁니다
...
### v1.5.0
- **채널 역할 안내 자동 게시**: Discord 채널이 새로 생성될 때 각 채널 첫 메시지로 목적, 담당 역할, 작성 내용, 운영 원칙을 상세히 안내
- **채널별 이모지 적용**: topic과 생성 로그에 역할별 이모지(👑📊🧭📅🏗️🧑‍💻⚙️🧪🤝✅)를 포함해 채널 용도를 직관적으로 표시
- **중복 스팸 방지**: 이미 존재하는 채널에는 안내 메시지를 다시 보내지 않고 신규 생성 채널에만 게시
